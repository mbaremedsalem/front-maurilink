      ...
      >
        <Link to={`/jobs/${job.id}`} className="block h-full w-full flex flex-col">
            ...
            {logoUrl ? (
              ...
              <div className="w-full h-full bg-gradient-to-br from-blue-500 via-indigo-500 to-purple-600 flex flex-col items-center justify-center text-white">
                ...
                <p className="text-xs md:text-sm font-medium text-center px-4 opacity-90 line-clamp-2">
                  {job.company_details?.company_name || 'Entreprise'}
                </p>
              </div>
            )}
          ...
          <div className="p-4 flex-1 flex flex-col">
            ...
            <div className="flex items-center gap-2 text-gray-600 text-xs md:text-sm mb-2">
              <HiLocationMarker className="h-3.5 w-3.5 md:h-4 md:w-4 flex-shrink-0" />
              <span className="truncate">{job.location || 'Non spécifiée'}</span>
            </div>
            ...
            <div className="flex flex-wrap items-center justify-between gap-2 text-sm mb-3">
              ...
              <div className="flex items-center gap-1 text-gray-500">
                ...
                <span className="text-xs">
                  {new Date(job.published_date).toLocaleDateString('fr-FR', { day: 'numeric', month: 'short' })}
                </span>
              </div>
            </div>
          ...
          {/* Footer */}
          <div className="px-4 pb-4 pt-3 border-t border-gray-100 mt-auto">
      ...
      <Link to={`/jobs/${job.id}`} className="block w-full h-full">
        <div className="flex flex-col sm:flex-row w-full">
          ...
          <div className="relative w-full sm:w-48 md:w-56 lg:w-64 h-48 sm:h-auto aspect-[16/9] sm:aspect-auto overflow-hidden bg-gradient-to-br from-gray-100 to-gray-200 flex-shrink-0">
            ...
            <div className="absolute top-3 left-3 right-3 flex flex-wrap gap-2 z-10 sm:hidden">
              <span className={`px-2 py-1 rounded-lg text-xs font-semibold backdrop-blur-md shadow-md ${getContractTypeColor(job.contract_type)}`}>
                {job.contract_type}
              </span>
              {isNew && (
                ...
                <div className="flex flex-wrap justify-between items-center gap-3">
                  <div className="flex flex-wrap items-center gap-3 md:gap-4">
                    ...
                    <div className="flex items-center gap-1 text-gray-500 text-xs md:text-sm">
                      ...
                      <span>Publiée le {new Date(job.published_date).toLocaleDateString('fr-FR')}</span>
                    </div>
                  </div>
